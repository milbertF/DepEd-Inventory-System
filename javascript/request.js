...
window.scrollToRequest = function(requestId) {
    if (!requestId || isRedirectingToRequest) {
        return;
    }
...
function resetRequestFiltersForScroll(requestId) {
    ...
    setTimeout(() => {
        attemptRequestScroll(requestId);
    }, 300);
}
...
function scrollToRequestOnCurrentPage(requestId) {
    ...
    if (targetRow && targetRow.style.display !== 'none') {
        ...
        setTimeout(() => {
            targetRow.classList.remove('highlight-request-strong');
        }, 3000);
...
function initStatusFilterAutoSelect() {
    const urlParams = new URLSearchParams(window.location.search);
    ...
    if (statusFromURL) {
        ...
        const targetCheckbox = document.querySelector(`.status-checkbox[data-status="${statusFromURL.toLowerCase()}"]`);
        if (targetCheckbox) {
            targetCheckbox.checked = true;
            
            requestCurrentFilters.status = [statusFromURL.toLowerCase()];
            saveStatusFilters();
            filterRequests();
...
function updateRequestRowNumbers() {
    ...
    const endIndex = Math.min(startIndex + requestRowsPerPage, filteredRequestRows.length);
    
    for (let i = startIndex; i < endIndex; i++) {
        const row = filteredRequestRows[i];
